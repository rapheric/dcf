...
const findByDeferralNumber = (deferralNumber) => {
  ...
  console.info(
    `[localDeferralService] findByDeferralNumber: no match for '${deferralNumber}' (searched ${all.length} items)`
  );
  return null;
};
...
const updateDeferral = (updated) => {
  ...
  const next = all.map((d) =>
    d._id === updated._id || d.deferralNumber === updated.deferralNumber
      ? { ...d, ...updated }
      : d
  );
  writeAll(next);
